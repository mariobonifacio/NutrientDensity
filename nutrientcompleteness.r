nutrientcompleteness <- function(){

## This tool summarized the entire SR27
## into individual nutrient density and
## nutrient completeness scores
...
temp <- data.frame(nd=1:8618,nc=1:8618)
temp[1] <- NA
...
## This if statement limits the output to just
## the raw foods
...
nccomponents <- ifelse(pctrdi <= .05, 0, 1)
mydata[i,54] <- sum(nccomponents,na.rm = TRUE)

ndcomponents <- ifelse(pctrdi <= .05, 0, log10(pctrdi/.05))
mydata[i,55] <- sum(ndcomponents,na.rm = TRUE)

ndsummary[i,"food"] <- test$Shrt_Desc
ndsummary[i,"nc"] <- sum(nccomponents,na.rm = TRUE)
ndsummary[i,"nd"] <- sum(ndcomponents,na.rm = TRUE)
ndsummary[i,"nckalescale"] <- round(100 * sum(nccomponents,na.rm = TRUE) / 19)
ndsummary[i,"ndkalescale"] <- round(100 * sum(ndcomponents,na.rm = TRUE) / 17.4691468)
...
ndsummary <- ndsummary[order(-ndsummary$nc,-ndsummary$nd),]



write.csv(ndsummary,"abbrev5.csv")
